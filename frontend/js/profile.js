(async function () {
    ...
    content.innerHTML = `
        ...
        <div class="card profile-identity" id="profile-identity"><div class="profile-avatar" id="profile-avatar">${fallbackInitial}</div><h3 id="profile-name">${user.username}</h3><p id="profile-handle">${brand.profileLabel || "Panel account"}</p><span class="pill connected">${user.status}</span></div>
        <div class="card">
            ...
            <table>
                ...
                <tr><td>Expired</td><td>${user.expiredAt ? new Date(user.expiredAt).toLocaleString("id-ID") : "Lifetime"}</td></tr>
            </table>
        </div>
