    ...
    function sessionRow(s) {
        ...
        return `
            <tr data-sender-row data-number="${s.number}" data-owner="${s.ownerUsername || s.ownerUserId || ""}" data-visibility="${s.isGlobal ? "global" : "personal"}" data-status="${st}">
                <td>${s.number}</td>
                ${isOwner ? `<td>${s.ownerUsername || s.ownerUserId || "-"}</td>` : ""}
                ${isOwner ? `<td><select class="sender-visibility-select" data-visibility-number="${s.number}" data-visibility-user="${s.ownerUserId}"><option value="personal" ${s.isGlobal ? "" : "selected"}>Personal</option><option value="global" ${s.isGlobal ? "selected" : ""}>Global</option></select></td>` : ""}
                <td><span class="pill ${pillClass(st)}" title="${s.statusReason || ""}">${st}</span><small class="sender-status-reason">${s.statusReason || ""}</small></td>
                <td>${s.createdAt ? new Date(s.createdAt).toLocaleDateString("en-US") : "-"}</td>
                <td><button class="btn secondary" data-reconnect="${s.number}" data-reconnect-user="${s.ownerUserId || user.id}">Reconnect</button> <button class="btn danger" data-remove="${s.number}" data-remove-user="${s.ownerUserId || user.id}">Remove</button></td>
            </tr>
        `;
    }
    ...
    async function loadSessions() {
        ...
        try {
            ...
            listBox.innerHTML = rows.length
                ? `<div class="sender-list-tools"><input id="sender-search" type="search" placeholder="Search sender or owner"><select id="sender-status-filter"><option value="">All statuses</option><option value="connected">Connected</option><option value="connecting">Connecting</option><option value="closed">Closed</option><option value="disconnected">Disconnected</option></select>${isOwner ? '<select id="sender-visibility-filter"><option value="">All visibility</option><option value="global">Global</option><option value="personal">Personal</option></select>' : ""}<button class="btn secondary" id="refresh-senders">Refresh</button></div><table><thead><tr><th>Number</th>${isOwner ? "<th>Owner</th><th>Visibility</th>" : ""}<th>Status</th><th>Created</th><th></th></tr></thead>
            ...
            listBox.querySelectorAll("[data-remove]").forEach((btn) => {
                btn.addEventListener("click", async () => {
                    ...
                    try {
                        ...
                        document.getElementById("refresh-senders")?.addEventListener("click", loadSessions);
