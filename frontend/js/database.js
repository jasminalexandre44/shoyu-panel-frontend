(async function () {
    ...
    function renderTable(filtered) {
        ...
        return `<div class="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Telegram ID</th><th>Expires</th><th>Created</th><th></th></tr></thead><tbody>${filtered.map((entry) => {
            ...
            return `<tr><td><strong>${escapeHtml(entry.username)}</strong><small class="table-sub">${escapeHtml(entry.source)} account</small></td><td><span class="role-badge">${escapeHtml(entry.role)}</span></td><td><span class="status-pill ${statusClass}">${escapeHtml(status)}</span></td><td>${escapeHtml(entry.telegramId || "-")}</td><td>${formatDate(entry.expiredAt)}</td><td>${formatDate(entry.createdAt)}</td><td>${canDeleteThisEntry ? `<button class="btn danger small" data-delete-user="${escapeHtml(entry.id)}" data-username="${escapeHtml(entry.username)}">Delete</button>` : `<span class="table-protected">Protected</span>`}</td></tr>`;
