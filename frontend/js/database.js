(async function () {
            ...
            <div class="database-modal" id="create-user-modal" hidden>
                ...
                <form class="card database-form" id="create-user-form">
                    ...
                    <div class="database-form-grid">
                        ...
                        <label class="field"><span>Role</span><select name="role">${roles.filter((role) => role !== "OWNER").map((role) => `<option value="${escapeHtml(role)}">${escapeHtml(role)}</option>`).join("")}</select></label>
    ...
    async function loadUsers() {
        try {
            ...
            users = data.users || [];
            roles = data.roles || ["ADMIN", "RESELLER", "VVIP", "PREMIUM"];
            render();
        } catch (error) {
