        ...
        content.innerHTML = `
            ...
            <div class="card database-card">
                <div class="database-toolbar">
                    <div class="database-toolbar-copy"><span class="page-kicker">Account directory</span><h3>Accounts</h3><p>Passwords are never displayed after creation.</p></div>
                    <div class="database-filters"><label><span>Search</span><input id="database-search" type="search" placeholder="Username, role, Telegram ID"></label><label><span>Role</span><select id="database-role"><option value="">All roles</option>${roles.map((role) => `<option value="${escapeHtml(role)}">${escapeHtml(role)}</option>`).join("")}</select></label></div>
                </div>
