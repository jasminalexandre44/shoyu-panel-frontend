...
const NAV_ITEMS = [
    ...
    { href: "/xmessage.html", icon: "xmessage", label: "XMessage", sub: "Send messages", roles: ["OWNER", "ADMIN", "RESELLER", "VVIP", "PREMIUM"] },
...
function renderShell(user, activeHref) {
    ...
    const themeOptions = THEME_OPTIONS.map((theme) => `
        <button type="button" class="theme-option" data-theme="${theme.id}"><i style="--swatch:${theme.swatch}"></i><span>${theme.label}</span></button>
    `).join("");
    ...
    shell.innerHTML = `
        <aside class="sidebar" id="sidebar">
            <a class="sidebar-brand" href="/dashboard.html" aria-label="Open dashboard" title="Dashboard">
                ${logoMarkup}
                <span class="sidebar-brand-copy"><strong>${brand.webName || "SHOYU"}</strong><small>${brand.panelLabel || "Control panel"}</small></span>
            </a>
        ...
        <main class="main">
            <div class="topbar">
                ...
                <div class="topbar-actions"><div class="theme-menu-wrap"><button class="theme-toggle" id="theme-toggle" aria-label="Choose theme">Theme</button><div class="theme-menu" id="theme-menu" hidden>${themeOptions}</div></div><span class="role-badge">${user.role}</span></div>
            </div>
