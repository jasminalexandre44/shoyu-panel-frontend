...
function renderShell(user, activeHref) {
    ...
    `).join("");
    const themeOptions = THEME_OPTIONS.map((theme, index) => `
        <button type="button" class="theme-option ${index >= 9 ? "theme-option-extra" : ""}" data-theme="${theme.id}"><i style="--swatch:${theme.swatch}"></i><span>${theme.label}</span></button>
    `).join("");
