(async function () {
    ...
    try {
        ...
        try {
            ...
            canUseGlobal = globalSessions.length > 0;
        } catch {
            globalSessions = [];
            canUseGlobal = false;
        }
    ...
    function render() {
        ...
        content.innerHTML = `
            <div class="page-intro"><div><span class="page-kicker">Message operations</span><h2>XMessage</h2><p>Choose a sender and message type, then monitor delivery in real time.</p></div><a class="btn secondary" href="/whatsapp.html">Manage Senders</a></div>
            ...
            <div class="card xmessage-activity-card">
                <div class="section-heading"><div><h3>XMessage Activity</h3><p>Background jobs continue after the message is accepted.</p></div><button class="btn secondary" id="refresh-xmessage-log">Refresh</button></div>
        ...
        async function loadActivity() {
            ...
            try {
                ...
                activityList.innerHTML = data.jobs.length ? data.jobs.map(consoleJobMarkup).join("") : `<div class="sub">No active XMessage process.</div>`;
